...
SyncProtobufCodec::SyncProtobufCodec(const SyncMessageCallback& callback)
: syncMessageCallback_(callback)// messageCallback_(NULL)
{

};

SyncProtobufCodec::~SyncProtobufCodec()
{
	/*		google::protobuf::ShutdownProtobufLibrary();*/
};

shared_ptr_Msg SyncProtobufCodec::CreateMessage(const std::string& type_name)
{
...
// +--------------------------------------------------------------+	
shared_ptr_Msg SyncProtobufCodec::Decode(const char* buf, uint32_t len)
{
...
std::shared_ptr<MyBuffer> SyncProtobufCodec::Encode(const CMessage& message)
{
	...
	// 	const uint32_t bb = PeekUint32(strmessage.c_str()  + aa - kHeaderLen);//获取检验和
	// 	const uint32_t cc = adler32(1, reinterpret_cast<const Bytef*>(strmessage.c_str()), aa - kHeaderLen);
	return ptrBuffer;
};

uint32_t SyncProtobufCodec::PeekUint32(const char* buf)
{
	uint32_t be32 = 0;
	int i = sizeof(be32);
...
bool SyncProtobufCodec::IsCorrect(const char* buf, int32_t len)
{
...
void SyncProtobufCodec::onSyncMessage(const std::string& strmessage)
{
	//message length
