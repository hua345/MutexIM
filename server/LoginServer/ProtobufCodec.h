...
public:
	...
	typedef std::function<void(const TSessionPtr&,
		const shared_ptr_Msg&)> ProtobufMessageCallback;

	explicit ProtobufCodec(const ProtobufMessageCallback& callback);

	void Send(const TSessionPtr& connection,
		const google::protobuf::Message& message)
	{
		// FIXME: serialize to TcpConnection::outputBuffer()
		auto buf = Encode(message);
		connection->sslSend(buf);
	}

	~ProtobufCodec();
	void onMessage(const TSessionPtr& connection, const std::string& strmessage);
	//消息基类
